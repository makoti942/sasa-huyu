...
app.post('/api/oauth/exchange', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (accountsRes.ok) {
                const accountsData = await accountsRes.json() as { data?: Array<{ id: string; account_type?: string }> };
...
app.post('/api/trading/otp', async (req, res) => {
    ...
    const { accountId: bodyAccountId } = req.body as { accountId?: string };
    const accountId = bodyAccountId || (req.cookies[ACCOUNT_ID_COOKIE] as string | undefined);
...
app.get('/api/trading/accounts', async (req, res) => {
    ...
    try {
        const upstream = await fetch(`${DERIV_REST_BASE}/trading/v1/options/accounts`, {
            headers: {
                'Authorization': `Bearer ${accessToken}`,
                'Deriv-App-ID':  DERIV_APP_ID,
            },
